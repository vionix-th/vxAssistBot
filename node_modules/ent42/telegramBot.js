...
class CuteAiTelegramBot {
  ...
  isAuthorized(msg) {
    ...
    if (msg.chat.type === 'supergroup' || msg.chat.type === 'group') {
      if (msg.new_chat_members?.find((m) => m.id === this.botInfo.id) ? true : false) {
        debugOut(`Added to group ${msg.chat.id}/${msg.chat.title} by ${msg.from.id}/${msg.from.username} ${msg.from.first_name} ${msg.from.last_name}`);
        allowed = allowed && licence.type === 'group';
      }
    }
  ...
  async completeMessageConditional(msg) {
    ...
    const licence = this.isBotOwner(msg.from.id) || this.isBotAdmin(msg.from.id)
      ? Licensing.getRootLicense()
      : Licensing.getByConsumer(msg.from.id);
