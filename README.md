...
This repository now contains the vxAssistBot runtime plus the shared AI/provider utilities that the bot depends on.

Key files:

- `vxAssistBot.js`: bot entry point and command handlers
- `AIInterface.js`: chat, image, and speech abstraction over OpenAI, Hugging Face, and local TTS backends
- `AIOpenAI.js`: OpenAI image backend
- `AIHuggingFace.js`: Hugging Face image and TTS backend
- `AILocalSystem.js`: macOS `say`-based TTS backend
- `vxAssistCommon.js`: shared file, logging, parsing, and markup utilities
- `strings.en_US.json`: command names and descriptions
- `db/`: runtime bot state and caches

## Business-Facing Capabilities

The retained vxAssistBot functionality is relevant to customer support, internal automation, and guided AI interaction:
...
vxAssistBot stores separate AI state per chat and, for Telegram forums, per thread. That enables scoped behavior for different customers, teams, or work topics.

## Architecture

### Telegram Layer

`vxAssistBot.js` extends a local base implementation from `ent42/telegramBot.js`. It registers commands, performs access checks, handles context management, and delegates AI work to `AIInterface.js`.

### AI Layer

`AIInterface.js` is responsible for:
...
vxAssistBot persists runtime state directly in the repository working tree:
...
vxAssistBot exposes per-chat configuration through `/setparam`. Relevant parameters retained in this portfolio version include:
...
- the owner ID is hardcoded in `node_modules/ent42/abstractCubeAiBot.js`
